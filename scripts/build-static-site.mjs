...
function assertArchitectureContract() {
  ...
  if (!template.includes('window.WORKS_PORTFOLIO_SEARCH_INDEX = __WORKS_PORTFOLIO_SEARCH_INDEX__;')) {
    throw new Error('Source template must expose only the dedicated search-index placeholder.');
  }
  ...
  if (!search.includes('window.WORKS_PORTFOLIO_SEARCH') || !search.includes('data-header-search-input')) {
    throw new Error('home-shell.js must own Header Search and the shared search meaning.');
  }
}
...
function buildPublishedProjects() {
  ...
  const withPrivate = mergePrivateSafeSummaries(publishedPublic);
  const annotated = annotateTaxonomy(withPrivate);
  return { projects: pruneRelations(annotated), withheldCount: canonical.length - publishedPublic.length };
}

function searchIndexProject(project) {
  const hintSource = project.friction ? 'friction' : 'content';
  const hint = truncate(project.friction || project.summary || project.subtitle || '', 120);
  ...
  return {
    ...
    summaryOnly: Boolean(project.summaryOnly),
    sourceVisibility: project.sourceVisibility || '',
    aliases: unique(project.searchAliases || []),
    verbs: unique(project.verbs || []),
    technologies: unique(project.technologies || []),
    families: unique(project.portfolioFamilies || []),
    searchText
  };
}
...
function catalogProject(project) {
  return {
    id: project.id,
    title: project.title || project.id,
    summary: project.summary || project.friction || '制作物の説明を整理中。',
    type: project.type || 'other',
    status: project.status || 'legacy',
    startedAt: project.startedAt || project.createdAt || '',
...
function detailProject(project) {
  return {
    ...
    currentAnswer: project.currentAnswer || '',
    type: project.type || 'other',
    verbs: unique(project.verbs || []),
    status: project.status || 'legacy',
    startedAt: project.startedAt || project.createdAt || '',
...
function writeRuntimeData(projects, generatedAt) {
  ...
  const catalogPayload = {
    version: 1,
    generatedAt,
    projects: projects.map(catalogProject)
  };
  const catalogPath = join(root, 'data/catalog-projects.json');
  writeFileSync(catalogPath, `${JSON.stringify(catalogPayload)}\n`);
  ...
  return {
    searchIndex,
    searchBytes: byteLength(JSON.stringify(searchIndex)),
    catalogBytes: byteLength(JSON.stringify(catalogPayload)),
    detailCount,
    detailBytes
  };
}
...
html = applyHomeSections(html, projects);
html = html.replace('__WORKS_PORTFOLIO_SEARCH_INDEX__', scriptJson(runtimeData.searchIndex));
...
for (const required of ['WORKS_PORTFOLIO_SEARCH_INDEX', 'data-header-search-input', 'data-home-friction="reduce"', 'data-home-surprise', 'home-shell.js', 'catalog.js', 'project-detail.js']) {
  if (!html.includes(required)) throw new Error(`Generated page is missing ${required}.`);
}
for (const forbidden of ['BUILD_DIARY_DATA', 'WORKS_PORTFOLIO_REPOSITORIES', 'WORKS_PORTFOLIO_START_DATES', 'WORKS_PORTFOLIO_SHOWCASE', 'WORKS_PORTFOLIO_CONFIG', 'requestIdleCallback', 'MutationObserver', 'live-index.js', 'friction-atlas.js', 'catalog-list-first.js']) {
  if (html.includes(forbidden)) throw new Error(`Generated page leaked obsolete runtime architecture: ${forbidden}.`);
}

writeFileSync(join(root, 'index.html'), html);
console.log(
  `Generated progressive portfolio: ${projects.length} projects (${withheldCount} public withheld); ` +
  `index ${byteLength(html).toLocaleString('en-US')} bytes; inline search ${runtimeData.searchBytes.toLocaleString('en-US')} bytes; ` +
  `catalog ${runtimeData.catalogBytes.toLocaleString('en-US')} bytes; ${runtimeData.detailCount} on-demand details / ${runtimeData.detailBytes.toLocaleString('en-US')} bytes; assets ${assetVersion}.`
);
