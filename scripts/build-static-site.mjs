...
function assertArchitectureContract() {
  ...
  for (const forbidden of ['BUILD_DIARY_DATA', 'data-view-button="timeline"', 'data-view-button="shelf"', 'data-view-button="map"']) {
    if (template.includes(forbidden)) throw new Error(`Legacy source architecture returned: ${forbidden}`);
  }
...
function createAssetVersion(paths) {
  ...
  for (const path of paths) {
    hash.update(path); hash.update('\0'); hash.update(readFileSync(join(root, path))); hash.update('\0');
  }
...
function buildCanonicalProjects() {
  ...
  for (const repo of repositories) {
    ...
    const project = {
      ...
      friction: existing.friction || '', firstBuild: existing.firstBuild || '', currentAnswer: existing.currentAnswer || '',
      type: existing.type || inferType(repo), verbs: unique(existing.verbs || ['作る']), status: existing.status || (repo.archived ? 'dormant' : 'development'),
      visibility: existing.visibility || 'public', featured: Boolean(existing.featured), createdAt: existing.createdAt || createdAt,
      createdAtPrecision: existing.createdAtPrecision || 'day', updatedAt: updatedAt || existing.updatedAt || existing.createdAt || createdAt,
      startedAt: start.date || existing.startedAt || createdAt || existing.createdAt || '', startedAtPrecision: start.precision || existing.startedAtPrecision || existing.createdAtPrecision || 'day',
      startedAtBasis: start.basis || existing.startedAtBasis || (createdAt ? 'repository-created' : 'record-created'), repositoryUrl, liveUrl,
      technologies: unique([...(existing.technologies || []), ...(language ? [language] : [])]), documentationState: existing.documentationState || 'unreviewed',
      relatedProjects: Array.isArray(existing.relatedProjects) ? existing.relatedProjects : [], updates: Array.isArray(existing.updates) ? existing.updates : [], aside: existing.aside || '',
      searchAliases: unique(existing.searchAliases || []), portfolioFamilies: [], makingPrinciples: [], ...existing, id: projectId, repositoryUrl, liveUrl,
...
function annotateTaxonomy(projects) {
  ...
  for (const family of taxonomy.families || []) for (const id of family.projectIds || []) {
    const project = map.get(id); if (project && !project.portfolioFamilies.includes(family.label)) project.portfolioFamilies.push(family.label);
  }
  for (const principle of taxonomy.principles || []) for (const id of principle.projectIds || []) {
    const project = map.get(id); if (project && !project.makingPrinciples.includes(principle.label)) project.makingPrinciples.push(principle.label);
  }
  return projects;
}
...
function searchIndexProject(project) {
  ...
  const record = {
    ...
    h: hint,
    y: typeCodes[project.type] || 'o',
    u: project.updatedAt || project.createdAt || ''
  };
  if (project.friction) record.r = 1;
  if (project.liveUrl) record.l = project.liveUrl;
  if (project.summaryOnly) record.s = 1;
  const aliases = normalizedList(project.searchAliases || []);
  const verbs = normalizedList(project.verbs || []);
  const technologies = normalizedList(project.technologies || []);
  const families = normalizedList(project.portfolioFamilies || []);
...
function catalogProject(project) {
  return {
    id: project.id, title: project.title || project.id, summary: project.summary || project.friction || '制作物の説明を整理中。', type: project.type || 'other',
    ...
    liveUrl: project.liveUrl || '', repositoryUrl: project.repositoryUrl || '', sourceVisibility: project.sourceVisibility || '', summaryOnly: Boolean(project.summaryOnly)
  };
}

function detailProject(project) {
  return {
    id: project.id, title: project.title || project.id, subtitle: project.subtitle || '', summary: project.summary || '', friction: project.friction || '', firstBuild: project.firstBuild || '',
    currentAnswer: project.currentAnswer || '', type: project.type || 'other', verbs: unique(project.verbs || []), status: project.status || 'legacy', startedAt: project.startedAt || project.createdAt || '',
    createdAt: project.createdAt || '', updatedAt: project.updatedAt || project.createdAt || '', liveUrl: project.liveUrl || '', repositoryUrl: project.repositoryUrl || '', technologies: unique(project.technologies || []),
    documentationState: project.documentationState || 'unreviewed', relatedProjects: Array.isArray(project.relatedProjects) ? project.relatedProjects : [], updates: Array.isArray(project.updates) ? project.updates : [],
...
function writeRuntimeData(projects, generatedAt) {
  ...
  const detailDir = join(root, 'data/project-details');
  rmSync(detailDir, { recursive: true, force: true }); mkdirSync(detailDir, { recursive: true });
  let detailCount = 0; let detailBytes = 0;
  for (const project of projects) {
    if (!project?.id || project.summaryOnly) continue;
    const payload = JSON.stringify(detailProject(project));
    writeFileSync(join(detailDir, `${encodeURIComponent(String(project.id))}.json`), `${payload}\n`); detailCount += 1; detailBytes += byteLength(payload);
  }
...
function applyStaticShell(html) {
  const heroTitle = String(settings.heroTitle || '').trim(); const heroLead = String(settings.heroLead || '').trim();
...
function homeSectionsMarkup(projects) {
  const featuredMarkup = featuredProjects(projects).map((project) => `<article class="home-featured-card">
    ...
    <h3>${escapeHtml(project.title || project.id)}</h3><p>${escapeHtml(project.summary || project.friction || '制作物の説明を整理中。')}</p>
    <div class="home-featured-actions"><button type="button" data-home-open="${escapeAttr(project.id)}">制作記録を見る</button>${project.liveUrl ? `<a href="${escapeAttr(project.liveUrl)}" target="_blank" rel="noopener">公開ページ ↗</a>` : ''}</div>
  </article>`).join('');
  const frictionMarkup = HOME_FRICTIONS.map((theme) => {
    const count = projects.filter((project) => themeScore(project, theme) > 0).length;
    return `<button type="button" class="home-friction-button" data-home-friction="${theme.id}" aria-label="${escapeAttr(theme.label)}に関係する制作物 ${count}件を見る"><span>${theme.code}</span><strong>${escapeHtml(theme.label)}</strong><b>${count}</b></button>`;
  }).join('');
  return `<section class="home-start section-shell" aria-labelledby="home-start-title"><div class="home-section-head"><div><p class="eyebrow">START HERE</p><h2 id="home-start-title">いま見るなら</h2></div><p>全部を見る前に、方向の違う制作物を4つだけ。公開ページへ直接行くことも、制作記録を読むこともできます。</p></div><div class="home-featured-grid">${featuredMarkup}</div></section>
  <section class="home-frictions section-shell" aria-labelledby="home-frictions-title"><div class="home-section-head"><div><p class="eyebrow">WHY I MADE THEM</p><h2 id="home-frictions-title">何に困って作った？</h2></div><p>技術名ではなく、作る前にあった小さな引っかかりから制作物を探します。</p></div><div class="home-friction-grid">${frictionMarkup}</div><div class="home-surprise-row"><p>目的が決まっていないときは、過去の問題解決をひとつ引く。</p><button type="button" class="home-surprise" data-home-surprise>おまかせで1つ</button></div></section>`;
}
